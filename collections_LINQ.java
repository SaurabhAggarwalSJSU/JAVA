...
public class collections_LINQ {
	...
	public static void main(String args[]) {
		...
		ArrayList<ActorsClass> actor_arrayList = new ArrayList<ActorsClass>();
		actor_arrayList.add(new ActorsClass("a", 20));
		actor_arrayList.add(new ActorsClass("b", 90));

		// WHERE
		List<ActorsClass> abc = actor_arrayList.stream().filter(x -> x.age > 20).collect(Collectors.toList());
		...
		// GET MININMUM AGE
		int min_age = actor_arrayList.stream().min(Comparator.comparing(ActorsClass::getAge)).get().getAge();
		...
		// TO DICTIONARY:
		Map<String, Integer> map = actor_arrayList.stream().filter(x -> x.age > 0)
				.collect(Collectors.toMap(ActorsClass::getKey, ActorsClass::getValue));

		// PRINT A MAP:
		map.forEach((k, v) -> System.out.println(k + " => " + v));

		// GROUPBY:
		actor_arrayList.stream().filter(x -> x.age > 0).collect(Collectors.groupingBy(ActorsClass::getAge))
				.forEach((k, v) -> System.out.println(k + " => " + v.toString()));

		// GROUPBY: Type
		Map<Integer, List<ActorsClass>> grouping = actor_arrayList.stream().filter(x -> x.age > 0)
				.collect(Collectors.groupingBy(ActorsClass::getAge));
