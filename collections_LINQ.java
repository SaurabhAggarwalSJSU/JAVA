...
public class collections_LINQ {
	...
	public static void main(String args[]) {
		...
		Map<String, Integer> map = actor_arrayList.stream().filter(x -> x.age > 0)
				.collect(Collectors.toMap(ActorClass::getKey, ActorClass::getValue));

		// PRINT A MAP:
		map.forEach((k, v) -> System.out.println(k + " => " + v));

		// GROUPBY:
		actor_arrayList.stream().filter(x -> x.age > 0).collect(Collectors.groupingBy(ActorClass::getAge))
				.forEach((k, v) -> System.out.println(k + " => " + v.toString()));

		// GROUPBY: Type
		Map<Integer, List<ActorClass>> grouping = actor_arrayList.stream().filter(x -> x.age > 0)
				.collect(Collectors.groupingBy(ActorClass::getAge));
